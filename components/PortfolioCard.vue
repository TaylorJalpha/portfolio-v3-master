...
// useInView for this card; persist by uid (prefer _id, fallback to slug)
// Derive a stable uid for the item; fall back to slug/current or index/title to ensure uniqueness
const uid = props.item?._id || props.item?.slug?.current || props.item?.slug || props.item?.title || `index-${props.index ?? ''}`
// Use a slightly less negative rootMargin to ensure more cards are observed as they enter
const { root, inView } = useInView({ uid, once: true, rootMargin: '0px 0px -10% 0px' })

// Only allow BlurIn to animate once per card, ever (per session)
const enteredClass = ref(false)
let hasAnimated = false
try {
  const store = (globalThis as any).__useInViewEnteredMap as Map<string, boolean> | undefined
  const persisted = !!(uid && store && store.get(uid))
  if (!enteredClass.value && persisted) {
    requestAnimationFrame(() => { enteredClass.value = true })
    hasAnimated = true
  }
  // ...existing code...
} catch (e) {}
...
watch(inView, (v) => {
  // ...existing code...
  if (v && !enteredClass.value && !hasAnimated) {
    requestAnimationFrame(() => { enteredClass.value = true })
    hasAnimated = true
  }
}, { immediate: true })
